...
lv_obj_t *currentCondLabel, *currentTempLabel;
...
struct {
  ...
  String condition, sunriseTime, sunsetTime;
  float last24hRain, last24hHigh, last24hLow;
  float ytdHigh, ytdLow, ytdRain;
  int ytdRainyDays;
  struct { String time; float temp; String condition; float windSpeed, precip; } hourly[6];
  struct { String dayName, date; float high, low; String condition; float precip; } daily[7];
} weather;
...
void createUI() {
  ...
  lv_obj_set_style_text_font(currentCondLabel, &lv_font_montserrat_20, 0);
  lv_obj_set_style_text_color(currentCondLabel, lv_color_hex(COLOR_TEXT_PRIMARY), 0);
  lv_obj_align(currentCondLabel, LV_ALIGN_TOP_MID, 0, 80);
  ...
  for (int i = 0; i < 6; i++) {
    ...
    lv_obj_set_style_text_font(condL, &lv_font_montserrat_10, 0);
    lv_obj_set_style_text_color(condL, lv_color_hex(COLOR_TEXT_SECONDARY), 0);
    lv_obj_align(condL, LV_ALIGN_TOP_MID, 0, 45);
  ...
  for (int i = 0; i < 7; i++) {
    ...
    lv_obj_t *condL = lv_label_create(card);
    lv_obj_set_style_text_font(condL, &lv_font_montserrat_12, 0);
    lv_obj_set_style_text_color(condL, lv_color_hex(COLOR_TEXT_PRIMARY), 0);
    lv_obj_align(condL, LV_ALIGN_TOP_MID, 0, 35);
...
void updateWeather() {
  ...
  weather.condition = getWeatherCondition(doc["current"]["weather_code"]);
  ...
  for (int i = 0; i < 6; i++) {
    ...
    weather.hourly[i].condition = getWeatherCondition(doc["hourly"]["weather_code"][currentHour + offset]);
  ...
  for (int i = 0; i < 7; i++) {
    ...
    weather.daily[i].condition = getWeatherCondition(doc["daily"]["weather_code"][i]);
...
void updateWeatherDisplay() {
  ...
  lv_label_set_text(currentCondLabel, weather.condition.c_str());
  ...
  for (int i = 0; i < 6; i++) {
    ...
    lv_label_set_text(lv_obj_get_child(card, 2), weather.hourly[i].condition.c_str());
  ...
  for (int i = 0; i < 7; i++) {
    ...
    lv_label_set_text(lv_obj_get_child(card, 1), weather.daily[i].condition.c_str());
